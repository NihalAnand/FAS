...
<body>
				...
				<form style="margin-left: 100px;	 margin-top:140px; " action=beforeprintreceipt.php method="POST">
...
echo "<table style= 'padding-right:180px;' >";
	...
	";
		if ( mysqli_num_rows($result) > 0) {
			while ($row = mysqli_fetch_assoc($result)) {
				echo "<tr><td><a href='afterprintbutton.php?usn={$row["USN"]}'><i class='fa fa-wrench'aria-hidden='true' style='width: 20px;'></i>Print</td>
				<td>{$row['USN']}</td>
					<td>{$row['Name']}</td>
					<td>{$row['Phone']}</td>
					<td>{$row['DOB']}</td>
 ...
 echo "</table>";
	...
	";
			...
			while ($row = mysqli_fetch_assoc($result)) {
				echo "<tr><td><a href='afterprintbutton.php?usn={$row["USN"]}'><i class='fa fa-wrench'
aria-hidden='true' style='width: 20px;'></i>Print</td>
					<td>{$row['USN']}</td>
					<td>{$row['Name']}</td>
					<td>{$row['Phone']}</td>
					<td>{$row['DOB']}</td>
